import { useState, useEffect, useRef, useCallback } from "react";

// ─── RANDOMIZATION HELPERS ───
const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
const shuffle = (arr) => [...arr].sort(() => Math.random() - 0.5);
const randInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;

const caesarShift = (text, shift) => {
  let result = "";
  for (let c of text.toUpperCase()) {
    if (c >= "A" && c <= "Z") {
      result += String.fromCharCode(((c.charCodeAt(0) - 65 + shift + 26) % 26) + 65);
    } else {
      result += c;
    }
  }
  return result;
};

// ─── LEVEL GENERATORS (randomized each game) ───

const generateScanLevel = () => {
  const targets = [
    { ip: "192.168.1.42", ports: "22,8080", type: "SERVER" },
    { ip: "10.0.0.17", ports: "22,443,3306", type: "DATABASE" },
    { ip: "172.16.0.99", ports: "21,22,80", type: "WEB SERVER" },
  ];
  const target = pick(targets);
  const decoys = shuffle([
    { ip: "192.168.1.1", ports: "80,443", type: "ROUTER" },
    { ip: "192.168.1.10", ports: "22,80", type: "PRINTER" },
    { ip: "10.0.0.1", ports: "53,80", type: "DNS" },
    { ip: "172.16.0.1", ports: "80,443", type: "GATEWAY" },
    { ip: "192.168.1.25", ports: "554", type: "CAMERA" },
    { ip: "10.0.0.50", ports: "25,587", type: "MAIL" },
  ]).slice(0, 3);

  return {
    id: "scan",
    title: "RECONNAISSANCE",
    mission: "Scannez le réseau pour identifier la cible vulnérable. Tapez 'help' pour les commandes.",
    hint: `Tapez 'scan' puis connectez-vous à la cible marquée VULNÉRABLE.`,
    topic: "Scan réseau & Reconnaissance",
    targetIp: target.ip,
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║           COMMANDES DISPONIBLES              ║",
          "╠══════════════════════════════════════════════╣",
          "║  scan            - Scanner le réseau         ║",
          "║  connect <ip>    - Se connecter à une cible  ║",
          "║  nmap <ip>       - Détail des ports          ║",
          "║  mission / hint / score                      ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      scan: {
        response: [
          `⟳ Scanning réseau...`,
          ...decoys.map(
            (d) => `  ├─ ${d.ip.padEnd(16)} [${d.type}]${"".padEnd(10 - d.type.length)}Port ${d.ports}`
          ),
          `  └─ ${target.ip.padEnd(16)} [${target.type}]${"".padEnd(10 - target.type.length)}Port ${target.ports} ⚠ VULNÉRABLE`,
          "",
          `→ Cible détectée ! Tapez 'connect ${target.ip}'`,
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("connect ")) {
        const ip = input.slice(8).trim();
        if (ip === target.ip) return null;
        return [`✗ Connexion refusée à ${ip}. Ce n'est pas la bonne cible.`];
      }
      if (input.startsWith("nmap ")) {
        const ip = input.slice(5).trim();
        const all = [...decoys, target];
        const found = all.find((d) => d.ip === ip);
        if (found) {
          const vuln = found === target ? " ⚠ VULNÉRABLE" : "";
          return [`NMAP ${ip}:`, `  Type: ${found.type}`, `  Ports ouverts: ${found.ports}${vuln}`];
        }
        return [`✗ Hôte ${ip} non trouvé.`];
      }
      return null;
    },
    checkWin: (input) => input === `connect ${target.ip}`,
    winResponse: [
      `✓ Connexion établie avec ${target.ip}`,
      "",
      "⚠ LEÇON : La reconnaissance est la 1ère étape d'un pentest.",
      "→ Outils réels : Nmap, Masscan, Shodan.",
      "→ Défense : fermer les ports inutiles, IDS/IPS.",
    ],
  };
};

const generateBruteForceLevel = (difficulty) => {
  const passwords = {
    "débutant": [
      { pass: "admin", hint: "C'est le login par défaut le plus courant." },
      { pass: "password", hint: "Le mot de passe le plus utilisé au monde." },
      { pass: "123456", hint: "La suite de chiffres la plus évidente." },
    ],
    "intermédiaire": [
      { pass: "root", hint: "Compte superutilisateur Linux." },
      { pass: "toor", hint: "Root à l'envers — courant sur les distros de pentest." },
      { pass: "changeme", hint: "Message souvent laissé comme password par défaut." },
    ],
    "expert": [
      { pass: "P@ssw0rd", hint: "Le classique avec substitutions évidentes (leet speak basique)." },
      { pass: "Welcome1", hint: "Souvent utilisé comme password initial en entreprise." },
      { pass: "Summer2024", hint: "Pattern saisonnier+année, très courant en entreprise." },
    ],
  };

  const pool = passwords[difficulty];
  const chosen = pick(pool);
  const maxAttempts = difficulty === "débutant" ? 10 : difficulty === "intermédiaire" ? 7 : 5;
  let attempts = 0;

  const commonPasswords = shuffle([
    "123456", "password", "admin", "root", "qwerty", "letmein", "welcome",
    "monkey", "dragon", "master", "abc123", "111111", "toor", "changeme",
    "P@ssw0rd", "Welcome1", "Summer2024", "Winter2023", "Company1", "Azerty1",
  ]);

  return {
    id: "bruteforce",
    title: "BRUTE FORCE",
    mission: `Le serveur demande un mot de passe. Vous avez ${maxAttempts} tentatives. Testez avec 'try <password>'. Tapez 'wordlist' pour une liste.`,
    hint: chosen.hint,
    topic: "Mots de passe faibles & Brute force",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  try <mdp>     - Tester un mot de passe      ║",
          "║  wordlist      - Passwords courants           ║",
          "║  attempts      - Tentatives restantes         ║",
          "║  explain       - En savoir plus               ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      wordlist: {
        response: [
          "📋 Passwords fréquents :",
          ...commonPasswords.slice(0, 12).map((p, i) => `  ${(i + 1).toString().padStart(2)}. ${p}`),
          "",
          `⚠ En 2024, 80% des breaches impliquent des mots de passe faibles.`,
        ],
      },
      explain: {
        response: [
          "📖 BRUTE FORCE",
          "━━━━━━━━━━━━━━",
          "Tester systématiquement tous les mots de passe possibles.",
          "",
          "Variantes :",
          "  • Dictionnaire : tester une liste de mots courants",
          "  • Hybride : mots + variations (Password1, P@ssw0rd...)",
          "  • Pur : toutes les combinaisons (lent mais exhaustif)",
          "",
          `Outils réels : Hydra, John the Ripper, Hashcat.`,
          `Défense : rate limiting, 2FA, passwords 12+ caractères.`,
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("try ")) {
        const guess = input.slice(4).trim();
        attempts++;
        if (guess === chosen.pass) return null;
        const remaining = maxAttempts - attempts;
        if (remaining <= 0) {
          attempts = 0;
          return [
            `✗ Accès refusé. Plus de tentatives !`,
            `→ Le mot de passe était : ${chosen.pass}`,
            `→ Compteur réinitialisé, réessayez.`,
          ];
        }
        return [`✗ Accès refusé. (${remaining} tentatives restantes)`];
      }
      if (input === "attempts") {
        return [`Tentatives restantes : ${maxAttempts - attempts}/${maxAttempts}`];
      }
      if (input === "retry") {
        attempts = 0;
        return [`⟳ Compteur réinitialisé. ${maxAttempts} tentatives disponibles.`];
      }
      return null;
    },
    checkWin: (input) => input.startsWith("try ") && input.slice(4).trim() === chosen.pass,
    winResponse: [
      `✓ ACCÈS AUTORISÉ ! Password cracké : "${chosen.pass}"`,
      `  Trouvé en ${attempts + 1} tentative(s).`,
      "",
      "⚠ LEÇON :",
      "→ Ne jamais utiliser de passwords par défaut ou prévisibles.",
      "→ Minimum 12 caractères, mélange majuscules/minuscules/chiffres/symboles.",
      "→ Utiliser un gestionnaire de mots de passe (Bitwarden, KeePass).",
      "→ Activer le 2FA partout où c'est possible.",
    ],
  };
};

const generateCaesarLevel = (difficulty) => {
  const messages = {
    "débutant": ["ATTAQUE A MIDI", "MOT DE PASSE FRAGILE", "ALERTE SECURITE"],
    "intermédiaire": ["RENDEZ VOUS AU SERVEUR NORD", "TRANSFERT DE DONNEES EN COURS", "CLEF COMPROMISE CHANGER URGENT"],
    "expert": ["EXTRACTION FICHIER CONFIDENTIEL CONFIRME", "PROTOCOLE EVACUATION ACTIVE CODE ROUGE", "AGENT DOUBLE IDENTIFIE SECTEUR SEPT"],
  };

  const shifts = { "débutant": [3, 5], "intermédiaire": [7, 11, 13], "expert": [17, 19, 23] };

  const plaintext = pick(messages[difficulty]);
  const shift = pick(shifts[difficulty]);
  const ciphertext = caesarShift(plaintext, shift);

  return {
    id: "caesar",
    title: "CHIFFREMENT CÉSAR",
    mission: `Message intercepté : '${ciphertext}'. Décalage inconnu. Utilisez 'caesar <texte> <décalage>' pour tester.`,
    hint:
      difficulty === "débutant"
        ? `Essayez les petits décalages négatifs. Testez: caesar ${ciphertext} -${shift}`
        : `Il y a 25 décalages possibles. Utilisez 'bruteforce caesar' pour tous les tester.`,
    topic: "Cryptographie — Chiffrement de César",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  caesar <texte> <n>   - Appliquer décalage   ║",
          "║  bruteforce caesar    - Tester les 25 shifts  ║",
          "║  explain              - Comprendre César      ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      "bruteforce caesar": {
        response: [
          `⟳ Test des 25 décalages sur '${ciphertext.slice(0, 20)}${ciphertext.length > 20 ? "..." : ""}' :`,
          "",
          ...Array.from({ length: 25 }, (_, i) => {
            const s = -(i + 1);
            const result = caesarShift(ciphertext, s);
            const marker = result === plaintext ? " ◄◄◄ LISIBLE !" : "";
            return `  ROT ${String(s).padStart(3)} → ${result.slice(0, 40)}${result.length > 40 ? "..." : ""}${marker}`;
          }),
          "",
          "→ Repérez le texte lisible et validez avec caesar !",
        ],
      },
      explain: {
        response: [
          "📖 CHIFFREMENT DE CÉSAR",
          "━━━━━━━━━━━━━━━━━━━━━━━",
          "Chaque lettre décalée de N positions dans l'alphabet.",
          `Exemple ROT+3 : A→D, B→E, ... Z→C`,
          "",
          "Faiblesse : seulement 25 clés possibles.",
          "Systèmes modernes : AES-256 (2²⁵⁶ clés possibles).",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("caesar ")) {
        const parts = input.split(" ");
        const sh = parseInt(parts[parts.length - 1]);
        if (isNaN(sh)) return ["✗ Format: caesar <texte> <décalage>"];
        const text = parts.slice(1, -1).join(" ").toUpperCase();
        const result = caesarShift(text, sh);
        if (result === plaintext) return null;
        return [`→ ${result}`, "", "Pas le bon décalage..."];
      }
      return null;
    },
    checkWin: (input) => {
      if (!input.startsWith("caesar ")) return false;
      const parts = input.split(" ");
      const sh = parseInt(parts[parts.length - 1]);
      if (isNaN(sh)) return false;
      const text = parts.slice(1, -1).join(" ").toUpperCase();
      return caesarShift(text, sh) === plaintext;
    },
    winResponse: [
      `✓ MESSAGE DÉCHIFFRÉ : ${plaintext}`,
      `  Décalage utilisé : ${shift}`,
      "",
      "⚠ LEÇON :",
      "→ César est cassable en secondes (25 possibilités).",
      "→ L'analyse de fréquence casse aussi les substitutions simples.",
      "→ Le chiffrement moderne utilise des clés de 128-256 bits.",
    ],
  };
};

const generateBase64Level = (difficulty) => {
  const secrets = {
    "débutant": ["Serveur Secret", "Admin Panel", "Root Access"],
    "intermédiaire": ["mysql://admin:tiger@db.internal:3306", "api_key=sk-9f8e7d6c5b4a3210", "ssh-rsa AAAAB3NzaC1yc2EA"],
    "expert": [
      '{"user":"admin","role":"superadmin","token":"x7k9m2"}',
      "password_hash=5f4dcc3b5aa765d61d8327deb882cf99",
    ],
  };

  const secret = pick(secrets[difficulty]);
  const encoded = btoa(secret);

  return {
    id: "base64",
    title: "ENCODAGE BASE64",
    mission: `Données suspectes interceptées : '${encoded}'. Décodez-les avec 'decode <texte>'.`,
    hint: `Base64 se termine souvent par = ou ==. Tapez: decode ${encoded}`,
    topic: "Encodage vs Chiffrement",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  decode <texte>    - Décoder du Base64        ║",
          "║  encode <texte>    - Encoder en Base64        ║",
          "║  explain           - Comprendre Base64        ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      explain: {
        response: [
          "📖 BASE64",
          "━━━━━━━━━",
          "Convertit des données binaires en texte ASCII (A-Z, a-z, 0-9, +, /).",
          "",
          "⚠ CE N'EST PAS DU CHIFFREMENT !",
          "→ Pas de clé, pas de secret. Tout le monde peut décoder.",
          "→ On le trouve partout : emails, JWT tokens, images inline.",
          "",
          "Erreur courante : stocker des passwords en Base64.",
          "→ Utiliser bcrypt, Argon2 ou scrypt pour le hachage.",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("decode ")) {
        const b64 = input.slice(7).trim();
        try {
          const decoded = atob(b64);
          if (decoded === secret) return null;
          return [`→ Décodé : ${decoded}`];
        } catch {
          return ["✗ Texte Base64 invalide. Vérifiez la chaîne."];
        }
      }
      if (input.startsWith("encode ")) {
        const text = input.slice(7).trim();
        return [`→ Encodé : ${btoa(text)}`];
      }
      return null;
    },
    checkWin: (input) => {
      if (!input.startsWith("decode ")) return false;
      try {
        return atob(input.slice(7).trim()) === secret;
      } catch {
        return false;
      }
    },
    winResponse: [
      `✓ DONNÉES DÉCODÉES : ${secret}`,
      "",
      "⚠ LEÇON :",
      "→ Base64 = encodage RÉVERSIBLE sans clé.",
      "→ Ne JAMAIS l'utiliser pour protéger des données sensibles.",
      difficulty !== "débutant" ? "→ En pentest, toujours checker les headers/cookies/tokens pour du Base64." : "→ Utiliser du vrai chiffrement (AES) ou du hachage (bcrypt).",
    ],
  };
};

const generateSQLiLevel = (difficulty) => {
  const scenarios = {
    "intermédiaire": {
      table: "users",
      query: "SELECT * FROM users WHERE username='{INPUT}' AND password='{PASS}'",
      hint: "Terminez le username avec une quote et un commentaire SQL : admin' --",
    },
    "expert": {
      table: "accounts",
      query: "SELECT * FROM accounts WHERE login='{INPUT}' AND token=SHA256('{PASS}')",
      hint: "Même avec du hachage côté requête, l'injection sur le login fonctionne : admin' UNION SELECT * FROM accounts --",
    },
  };

  const sc = scenarios[difficulty] || scenarios["intermédiaire"];

  return {
    id: "sqli",
    title: "INJECTION SQL",
    mission: `Formulaire de login (table: ${sc.table}). Exploitez la faille SQL avec 'login <user> <pass>'.`,
    hint: sc.hint,
    topic: "Injection SQL (OWASP #1)",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  login <user> <pass> - Tenter une connexion   ║",
          "║  view query          - Voir la requête SQL    ║",
          "║  explain             - Comprendre SQLi        ║",
          "║  tables              - Lister les tables      ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      "view query": {
        response: [`📋 Requête SQL :`, "", `  ${sc.query}`, "", "→ Que se passe-t-il si vous injectez une quote (') ?"],
      },
      tables: {
        response: [`Tables : ${sc.table} (id, username/login, password, role)`, `         sessions (id, user_id, token)`, `         logs (id, action, timestamp)`],
      },
      explain: {
        response: [
          "📖 INJECTION SQL",
          "━━━━━━━━━━━━━━━━",
          "Manipuler les requêtes SQL via les champs de saisie.",
          "",
          "Techniques :",
          "  • Commentaire : admin' -- (ignore le reste de la requête)",
          "  • OR : ' OR '1'='1 (condition toujours vraie)",
          "  • UNION : récupérer d'autres tables",
          "",
          "Défense : prepared statements, ORM, WAF.",
          "Outils réels : sqlmap, Burp Suite.",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("login ")) {
        const payload = input.slice(6);
        if (payload.includes("' --") || payload.includes("'--") || payload.toLowerCase().includes("' or '1'='1") || payload.toLowerCase().includes("' or 1=1") || payload.toLowerCase().includes("union select")) return null;
        return ["✗ Identifiants incorrects. Essayez d'exploiter la requête SQL."];
      }
      return null;
    },
    checkWin: (input) => {
      if (!input.startsWith("login ")) return false;
      const p = input.slice(6).toLowerCase();
      return p.includes("' --") || p.includes("'--") || p.includes("' or '1'='1") || p.includes("' or 1=1") || p.includes("union select");
    },
    winResponse: [
      "✓ INJECTION SQL RÉUSSIE ! Connecté en tant qu'admin.",
      "",
      "⚠ LEÇON :",
      "→ TOUJOURS utiliser des requêtes préparées (prepared statements).",
      "→ Ne jamais concaténer des entrées utilisateur dans du SQL.",
      "→ Valider et sanitizer côté serveur.",
    ],
  };
};

const generatePhishingLevel = (difficulty) => {
  const emailSets = {
    "débutant": [
      { from: "[email protected]", subject: "Réunion vendredi", body: "Bonjour,\nRéunion déplacée à vendredi 14h, salle B204.\nCordialement, Marie", legit: true },
      { from: "[email protected]", subject: "⚠ URGENT : Compte suspendu !", body: "Cher client,\nActivité suspecte détectée. Cliquez ICI :\n→ http://banque-france.security-check.xyz/login\n\nSans action dans 24h, compte BLOQUÉ.", legit: false, indices: ["securite-banque.com ≠ site officiel", "URL: security-check.xyz → frauduleux", "Urgence artificielle + menace"] },
      { from: "[email protected]", subject: "Mise à jour sécurité", body: "Bonjour,\nMise à jour dispo sur https://support.microsoft.com\nPatch KB5034441.\nL'équipe IT", legit: true },
    ],
    "intermédiaire": [
      { from: "[email protected]", subject: "Action requise : vérification sécurité", body: "Bonjour,\nSuite à notre audit trimestriel :\n→ https://microsoft-365-security.com/verify\n\nCordialement,\nÉquipe Sécurité Microsoft", legit: false, indices: ["microsoft-365-security.com ≠ microsoft.com", "Le 'o' dans micr0soft est un zéro", "Microsoft ne demande jamais de vérifier par email"] },
      { from: "[email protected]", subject: "Formation Cybersécurité", body: "Bonjour,\nInscription confirmée pour le 15 mars.\nLien Teams : https://teams.microsoft.com/meet/abc123\n\nRH", legit: true },
      { from: "[email protected]", subject: "Document partagé", body: "Jean-Marc a partagé 'Budget_Q4.xlsx'.\n→ https://docs.google.com/spreadsheets/d/1a2b3c\n\nOuvrir dans Sheets", legit: true },
    ],
    "expert": [
      { from: "[email protected]", subject: "Re: Accès VPN - Ticket #4892", body: "Bonjour,\nSuite à votre ticket, nouveau client VPN :\n→ https://vpn-corporate.entreprise-solutions.net/download\n\nMerci d'installer avant vendredi.\n\nSupport IT - Nicolas", legit: false, indices: ["entreprise-solutions.net ≠ domaine interne", "Téléchargement de logiciel via email = suspect", "'Re:' sans conversation précédente = social engineering"] },
      { from: "[email protected]", subject: "Certificat SSL expire", body: "Bonjour admin,\nCertificat SSL expire dans 7 jours.\nRenouvellement : https://portal.azure.com/certificates\n\nDevOps Team", legit: true },
      { from: "[email protected]", subject: "Facture #INV-2024-0892", body: "Bonjour,\nFacture janvier : 4 250,00 €\n→ https://facturation.ovhcloud.com/invoices/892\n\nComptabilité OVH", legit: true },
    ],
  };

  const emails = emailSets[difficulty];
  const phishIndex = emails.findIndex((e) => !e.legit);

  return {
    id: "phishing",
    title: "DÉTECTION PHISHING",
    mission: `${emails.length} emails interceptés. Identifiez le phishing ! Tapez 'read <n>' puis 'report <n>'.`,
    hint: `Vérifiez domaines, URLs et ton. Le phishing est l'email #${phishIndex + 1}.`,
    topic: "Phishing & Ingénierie sociale",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  read <1-3>      - Lire un email             ║",
          "║  report <1-3>    - Signaler comme phishing   ║",
          "║  analyze <1-3>   - Analyser les headers      ║",
          "║  explain         - Comprendre le phishing    ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      explain: {
        response: [
          "📖 DÉTECTER LE PHISHING",
          "━━━━━━━━━━━━━━━━━━━━━━━",
          "🔴 Signaux d'alerte :",
          "  • Domaine d'expéditeur suspect (typosquatting)",
          "  • URL ne correspondant pas au domaine officiel",
          "  • Urgence artificielle / menaces",
          "  • 'Re:' sans conversation préalable",
          "",
          "🟢 Vérifications :",
          "  • Survoler les liens AVANT de cliquer",
          "  • Vérifier le domaine exact",
          "  • En cas de doute, contacter par un autre canal",
        ],
      },
    },
    defaultResponse: (input) => {
      const match = input.match(/^(read|report|analyze) (\d)$/);
      if (!match) return null;
      const action = match[1];
      const idx = parseInt(match[2]) - 1;
      if (idx < 0 || idx >= emails.length) return [`✗ Email #${idx + 1} n'existe pas.`];
      const email = emails[idx];

      if (action === "read") {
        return ["═══════════════════════════════", `📧 EMAIL #${idx + 1}`, `De: ${email.from}`, `Objet: ${email.subject}`, "───────────────────────────────", ...email.body.split("\n"), "═══════════════════════════════"];
      }
      if (action === "analyze") {
        const domain = email.from.split("@")[1];
        return [`🔍 Headers email #${idx + 1} :`, `  From: ${email.from}`, `  Domain: ${domain}`, `  SPF: ${email.legit ? "PASS" : "SOFTFAIL"}`, `  DKIM: ${email.legit ? "PASS" : "NONE"}`, `  DMARC: ${email.legit ? "PASS" : "FAIL"}`, !email.legit ? "  ⚠ Authentification échouée !" : ""];
      }
      if (action === "report") {
        if (!email.legit) return null;
        return [`✗ L'email #${idx + 1} est légitime. Cherchez mieux !`];
      }
      return null;
    },
    checkWin: (input) => {
      const match = input.match(/^report (\d)$/);
      if (!match) return false;
      return parseInt(match[1]) - 1 === phishIndex;
    },
    winResponse: [
      `✓ CORRECT ! Phishing identifié !`,
      "",
      "🔍 Indices :",
      ...emails[phishIndex].indices.map((i) => `  • ${i}`),
      "",
      "⚠ LEÇON :",
      "→ 91% des cyberattaques commencent par du phishing.",
      "→ Toujours vérifier domaine + URL.",
      "→ En entreprise, utiliser le bouton 'Report Phishing'.",
    ],
  };
};

const generateHashLevel = () => {
  const hashes = [
    { plain: "password", md5: "5f4dcc3b5aa765d61d8327deb882cf99" },
    { plain: "admin", md5: "21232f297a57a5a743894a0e4a801fc3" },
    { plain: "letmein", md5: "0d107d09f5bbe40cade3de5c71e9e9b7" },
  ];
  const chosen = pick(hashes);

  return {
    id: "hash",
    title: "RAINBOW TABLES",
    mission: `Hash MD5 intercepté : '${chosen.md5}'. Retrouvez le mot de passe original avec 'rainbow <hash>' ou 'crack <hash>'.`,
    hint: "Les rainbow tables contiennent des millions de hash pré-calculés. Tapez: rainbow " + chosen.md5,
    topic: "Hachage & Rainbow Tables",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  rainbow <hash>  - Chercher dans rainbow table║",
          "║  crack <hash>    - Attaque dictionnaire       ║",
          "║  hash <texte>    - Calculer le hash d'un texte║",
          "║  explain         - Comprendre le hachage      ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      explain: {
        response: [
          "📖 HACHAGE",
          "━━━━━━━━━━",
          "Fonction à sens unique : texte → empreinte fixe.",
          "  MD5    : 128 bits (CASSÉ)",
          "  SHA-1  : 160 bits (CASSÉ)",
          "  SHA-256: 256 bits (sûr)",
          "  bcrypt : hachage + sel + coût variable (RECOMMANDÉ)",
          "",
          "Rainbow table = base de hash pré-calculés.",
          "Défense : sel (salt) unique par password.",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("rainbow ") || input.startsWith("crack ")) {
        const h = input.split(" ").slice(1).join(" ").trim();
        if (h === chosen.md5) return null;
        return ["✗ Hash non trouvé dans la rainbow table."];
      }
      if (input.startsWith("hash ")) {
        const text = input.slice(5).trim();
        const fakeHash = Array.from({ length: 32 }, () => "0123456789abcdef"[randInt(0, 15)]).join("");
        if (text === chosen.plain) return [`→ MD5(${text}) = ${chosen.md5}`, "C'est le même hash !"];
        return [`→ MD5(${text}) = ${fakeHash}`];
      }
      return null;
    },
    checkWin: (input) => {
      if (!input.startsWith("rainbow ") && !input.startsWith("crack ")) return false;
      return input.split(" ").slice(1).join(" ").trim() === chosen.md5;
    },
    winResponse: [
      `⟳ Recherche rainbow table...`,
      `  ${chosen.md5} → "${chosen.plain}"`,
      "",
      `✓ PASSWORD TROUVÉ : ${chosen.plain}`,
      "",
      "⚠ LEÇON :",
      "→ MD5 et SHA-1 sont CASSÉS pour le stockage de passwords.",
      "→ Défense : bcrypt/Argon2 avec un sel unique par utilisateur.",
    ],
  };
};

const generateOSINTLevel = () => {
  const targets = [
    { name: "Alex Morin", username: "alexm_dev", bio: "Full-stack dev 🚀 | Paris | Cat lover 🐱 | Born 15/03/1995", posts: ["Just deployed on AWS us-east-1 ! 🎉", "Birthday dinner at Le Petit Cler 🎂", "My cat Pixel turned 3 today !", "Working from Café de Flore ☕"], password: "Pixel1503", hint: "Prénom du chat + date naissance JJMM." },
    { name: "Sarah Benali", username: "sarah.benali", bio: "Marketing @TechCorp | Lyon | 🏃‍♀️ Marathon Paris 2024 | Dog mom to Oscar", posts: ["Oscar fête ses 5 ans ! 🐕", "Paris Marathon in 4h12 ! Personal best !", "Throwback wedding 07/07/2019 💕", "Working late at Lyon office..."], password: "Oscar0707", hint: "Nom du chien + date importante JJMM." },
  ];
  const target = pick(targets);

  return {
    id: "osint",
    title: "OSINT - RENSEIGNEMENT",
    mission: `Cible : ${target.name} (@${target.username}). Trouvez son mot de passe via ses réseaux sociaux. Commande: 'try <password>'.`,
    hint: target.hint,
    topic: "OSINT & Ingénierie Sociale",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  profile          - Voir le profil social     ║",
          "║  posts            - Voir les publications     ║",
          "║  try <password>   - Tester un mot de passe    ║",
          "║  explain          - Comprendre l'OSINT        ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      profile: {
        response: [`👤 @${target.username}`, `   ${target.name}`, `   Bio: ${target.bio}`, `   Followers: ${randInt(200, 2000)}`, "", "→ Tapez 'posts' pour les publications."],
      },
      posts: {
        response: [`📱 @${target.username} :`, "", ...target.posts.map((p, i) => `  ${["🕐 2h", "🕑 1j", "🕒 3j", "🕓 1sem"][i]} │ ${p}`), "", "→ Cherchez des indices personnels."],
      },
      explain: {
        response: [
          "📖 OSINT (Open Source Intelligence)",
          "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
          "Collecte d'infos à partir de sources publiques.",
          "  • Noms d'animaux → passwords courants",
          "  • Dates perso → codes PIN",
          "  • Lieux → questions de sécurité",
          "",
          "Outils : Maltego, theHarvester, Sherlock, Google Dorks.",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("try ")) {
        const guess = input.slice(4).trim();
        if (guess === target.password) return null;
        return ["✗ Mot de passe incorrect. Analysez mieux le profil."];
      }
      return null;
    },
    checkWin: (input) => input.startsWith("try ") && input.slice(4).trim() === target.password,
    winResponse: [
      `✓ ACCÈS ! Password : "${target.password}"`,
      "",
      "⚠ LEÇON :",
      "→ Ne JAMAIS utiliser d'infos personnelles dans un password.",
      "→ Limiter les infos partagées sur les réseaux sociaux.",
      "→ Utiliser un générateur de mots de passe aléatoires.",
    ],
  };
};

const generateNetworkLevel = () => {
  const suspicious = "192.168.1.105";
  const malicious = "185.143.223.1";
  const logs = [
    "10:01:03 | 192.168.1.100 → 192.168.1.1   | TCP | Port 80   | HTTP GET /index",
    "10:01:05 | 192.168.1.100 → 8.8.8.8        | UDP | Port 53   | DNS google.com",
    "10:01:07 | 192.168.1.105 → 45.33.32.156   | TCP | Port 443  | TLS Hello",
    `10:01:09 | ${suspicious} → ${malicious}  | TCP | Port 4444 | DATA ████████`,
    "10:01:11 | 192.168.1.100 → 192.168.1.1    | TCP | Port 443  | HTTPS /api",
    `10:01:13 | ${malicious} → ${suspicious}  | TCP | Port 4444 | DATA ████████`,
    "10:01:15 | 192.168.1.100 → 192.168.1.50   | TCP | Port 22   | SSH",
  ];

  return {
    id: "network",
    title: "ANALYSE RÉSEAU",
    mission: "Analysez les logs réseau. Identifiez la machine compromise avec 'flag <ip>'. Tapez 'logs' pour voir le trafic.",
    hint: `Port 4444 = Metasploit reverse shell. Tapez: flag ${suspicious}`,
    topic: "Analyse de trafic réseau",
    commands: {
      help: {
        response: [
          "╔══════════════════════════════════════════════╗",
          "║  logs             - Voir les logs réseau      ║",
          "║  whois <ip>       - Info sur une IP           ║",
          "║  flag <ip>        - Identifier la machine     ║",
          "║  explain          - Comprendre l'analyse      ║",
          "╚══════════════════════════════════════════════╝",
        ],
      },
      logs: {
        response: [
          "📋 CAPTURE RÉSEAU :",
          "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
          ...logs,
          "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
          "",
          "→ Identifiez l'activité suspecte.",
        ],
      },
      explain: {
        response: [
          "📖 ANALYSE RÉSEAU",
          "━━━━━━━━━━━━━━━━━",
          "Ports courants : 22=SSH, 53=DNS, 80=HTTP, 443=HTTPS",
          "Ports suspects : 4444=Metasploit, 5555=ADB, 1337=leet",
          "",
          "Signaux d'alerte :",
          "  • Ports non-standard vers IP externes",
          "  • Données chiffrées sur ports inhabituels",
          "  • Communication bidirectionnelle suspecte",
          "",
          "Outils : Wireshark, tcpdump, Zeek, Suricata.",
        ],
      },
    },
    defaultResponse: (input) => {
      if (input.startsWith("whois ")) {
        const ip = input.slice(6).trim();
        if (ip === malicious) return [`🔍 WHOIS ${ip} :`, `  Pays: Russie`, `  Org: Bulletproof Hosting Ltd`, `  ⚠ IP sur listes de menaces !`];
        if (ip === suspicious) return [`🔍 WHOIS ${ip} :`, `  Réseau local`, `  Host: DESKTOP-PC105`, `  User: j.martin`];
        return [`🔍 WHOIS ${ip} :`, `  Réseau local (privé)`];
      }
      if (input.startsWith("flag ")) {
        const ip = input.slice(5).trim();
        if (ip === suspicious) return null;
        return [`✗ ${ip} ne semble pas compromise. Analysez les ports.`];
      }
      return null;
    },
    checkWin: (input) => input.startsWith("flag ") && input.slice(5).trim() === suspicious,
    winResponse: [
      `✓ MACHINE COMPROMISE : ${suspicious}`,
      `  Reverse shell Metasploit (port 4444) vers ${malicious}`,
      "",
      "⚠ LEÇON :",
      "→ Surveiller les connexions sortantes sur ports non-standard.",
      "→ Les reverse shells utilisent souvent 4444, 1234, 8888.",
      "→ Outils : Wireshark, Suricata, Splunk SIEM.",
    ],
  };
};

// ─── PROFILES ───
const PROFILES = {
  "débutant": {
    label: "🟢 DÉBUTANT",
    desc: "Lycéen / Curieux — Découverte des bases",
    color: "#0f0",
    levels: () => [generateScanLevel(), generateBruteForceLevel("débutant"), generateCaesarLevel("débutant"), generateBase64Level("débutant"), generatePhishingLevel("débutant")],
  },
  "intermédiaire": {
    label: "🟡 INTERMÉDIAIRE",
    desc: "Étudiant IT / Bac+2 — Pentest basique",
    color: "#ff0",
    levels: () => [generateScanLevel(), generateBruteForceLevel("intermédiaire"), generateCaesarLevel("intermédiaire"), generateBase64Level("intermédiaire"), generateSQLiLevel("intermédiaire"), generateOSINTLevel(), generatePhishingLevel("intermédiaire")],
  },
  "expert": {
    label: "🔴 EXPERT",
    desc: "M1/M2 Cyber — Challenges avancés",
    color: "#f44",
    levels: () => [generateScanLevel(), generateBruteForceLevel("expert"), generateCaesarLevel("expert"), generateBase64Level("expert"), generateHashLevel(), generateSQLiLevel("expert"), generateOSINTLevel(), generateNetworkLevel(), generatePhishingLevel("expert")],
  },
};

// ─── MATRIX RAIN ───
const MatrixRain = () => {
  const canvasRef = useRef(null);
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    const resize = () => { canvas.width = canvas.parentElement.offsetWidth; canvas.height = canvas.parentElement.offsetHeight; };
    resize();
    const chars = "01アイウエオカキクケコサシスセソ";
    const fontSize = 14;
    const columns = Math.floor(canvas.width / fontSize);
    const drops = Array(columns).fill(1);
    const draw = () => {
      ctx.fillStyle = "rgba(0, 0, 0, 0.05)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.fillStyle = "#0f0";
      ctx.font = `${fontSize}px monospace`;
      for (let i = 0; i < drops.length; i++) {
        ctx.globalAlpha = Math.random() * 0.3 + 0.1;
        ctx.fillText(chars[Math.floor(Math.random() * chars.length)], i * fontSize, drops[i] * fontSize);
        if (drops[i] * fontSize > canvas.height && Math.random() > 0.975) drops[i] = 0;
        drops[i]++;
      }
      ctx.globalAlpha = 1;
    };
    const interval = setInterval(draw, 50);
    window.addEventListener("resize", resize);
    return () => { clearInterval(interval); window.removeEventListener("resize", resize); };
  }, []);
  return <canvas ref={canvasRef} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", opacity: 0.12, pointerEvents: "none" }} />;
};

// ─── MAIN GAME ───
export default function HackerGame() {
  const [screen, setScreen] = useState("menu");
  const [profile, setProfile] = useState(null);
  const [levels, setLevels] = useState([]);
  const [levelIdx, setLevelIdx] = useState(0);
  const [lines, setLines] = useState([]);
  const [input, setInput] = useState("");
  const [hintsUsed, setHintsUsed] = useState(0);
  const [startTime, setStartTime] = useState(null);
  const [playerName, setPlayerName] = useState("");
  const [leaderboard, setLeaderboard] = useState([]);
  const termRef = useRef(null);
  const inputRef = useRef(null);

  const scrollToBottom = useCallback(() => {
    if (termRef.current) setTimeout(() => { termRef.current.scrollTop = termRef.current.scrollHeight; }, 50);
  }, []);

  useEffect(() => { scrollToBottom(); }, [lines, scrollToBottom]);

  const startGame = (profileKey) => {
    const p = PROFILES[profileKey];
    const gen = p.levels();
    setProfile(profileKey);
    setLevels(gen);
    setLevelIdx(0);
    setHintsUsed(0);
    setStartTime(Date.now());
    setLines([
      { type: "system", text: `══════ NIVEAU 1/${gen.length} : ${gen[0].title} ══════` },
      { type: "system", text: `📡 ${gen[0].topic}` },
      { type: "mission", text: gen[0].mission },
      { type: "empty", text: "" },
    ]);
    setScreen("game");
  };

  const addLines = (newLines, type = "response") => setLines((prev) => [...prev, ...newLines.map((text) => ({ type, text }))]);

  const advanceLevel = () => {
    if (levelIdx < levels.length - 1) {
      const next = levelIdx + 1;
      setLevelIdx(next);
      setTimeout(() => {
        addLines(["", `══════ NIVEAU ${next + 1}/${levels.length} : ${levels[next].title} ══════`, `📡 ${levels[next].topic}`], "system");
        addLines([levels[next].mission], "mission");
        addLines([""], "empty");
      }, 800);
    } else {
      setTimeout(() => setScreen("victory"), 1000);
    }
  };

  const handleCommand = (cmd) => {
    const original = cmd.trim();
    const trimmed = original.toLowerCase();
    if (!trimmed) return;
    setLines((prev) => [...prev, { type: "input", text: `❯❯ ${original}` }]);

    if (trimmed === "clear") { setLines([]); return; }
    if (trimmed === "mission") { addLines([levels[levelIdx].mission], "mission"); return; }
    if (trimmed === "hint") { setHintsUsed((h) => h + 1); addLines(["💡 " + levels[levelIdx].hint], "hint"); return; }
    if (trimmed === "score") {
      const e = Math.floor((Date.now() - startTime) / 1000);
      addLines([`Niveau: ${levelIdx + 1}/${levels.length} | Hints: ${hintsUsed} | Temps: ${Math.floor(e / 60)}m${String(e % 60).padStart(2, "0")}s`], "system");
      return;
    }

    const cl = levels[levelIdx];
    const proc = (trimmed.startsWith("decode ") || trimmed.startsWith("caesar ") || trimmed.startsWith("login ") || trimmed.startsWith("try "))
      ? original.replace(/^\S+/, (m) => m.toLowerCase()) : trimmed;

    if (cl.checkWin && cl.checkWin(proc)) { if (cl.winResponse) addLines(cl.winResponse, "success"); advanceLevel(); return; }
    if (cl.commands[trimmed]) { const r = cl.commands[trimmed]; addLines(r.response, r.nextLevel ? "success" : "response"); if (r.nextLevel) advanceLevel(); return; }
    if (cl.defaultResponse) { const r = cl.defaultResponse(proc); if (r) { addLines(r, "response"); return; } }
    addLines(["Commande non reconnue. Tapez 'help'."], "error");
  };

  const handleKeyDown = (e) => { if (e.key === "Enter") { handleCommand(input); setInput(""); } };
  const focusInput = () => inputRef.current?.focus();
  const getScore = () => {
    const e = Math.floor((Date.now() - startTime) / 1000);
    const base = { "débutant": 500, "intermédiaire": 750, "expert": 1000 }[profile] || 500;
    return Math.max(0, base - hintsUsed * 50 - Math.floor(e / 15));
  };

  // ─── MENU ───
  if (screen === "menu") {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Fira Code','Cascadia Code',monospace", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
        <MatrixRain />
        <div style={{ zIndex: 10, textAlign: "center", maxWidth: "700px", padding: "20px" }}>
          <div style={{ color: "#0a0", fontSize: "11px", letterSpacing: "8px", marginBottom: "16px", opacity: 0.5 }}>ÉCOLE IT VALENCIENNES</div>
          <h1 style={{ color: "#0f0", fontSize: "38px", margin: "0 0 6px", textShadow: "0 0 30px rgba(0,255,0,0.4)", letterSpacing: "4px" }}>CYBER TERMINAL</h1>
          <div style={{ color: "#0a0", fontSize: "13px", letterSpacing: "5px", marginBottom: "40px", opacity: 0.7 }}>HACKING ÉTHIQUE — V2</div>
          <div style={{ marginBottom: "30px" }}>
            <input value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="Votre pseudo (optionnel)" maxLength={20}
              style={{ background: "rgba(0,255,0,0.05)", border: "1px solid #1a3a1a", color: "#0f0", padding: "10px 16px", fontSize: "14px", fontFamily: "inherit", textAlign: "center", borderRadius: "4px", outline: "none", width: "250px" }}
              onFocus={(e) => e.target.style.borderColor = "#0f0"} onBlur={(e) => e.target.style.borderColor = "#1a3a1a"} />
          </div>
          <div style={{ color: "#888", fontSize: "13px", marginBottom: "24px", letterSpacing: "2px" }}>CHOISISSEZ VOTRE NIVEAU</div>
          <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
            {Object.entries(PROFILES).map(([key, p]) => (
              <button key={key} onClick={() => startGame(key)}
                style={{ background: "rgba(0,0,0,0.8)", border: `1px solid ${p.color}33`, color: p.color, padding: "20px 24px", fontFamily: "inherit", fontSize: "13px", cursor: "pointer", borderRadius: "4px", width: "200px", textAlign: "left", transition: "all 0.3s", boxShadow: `0 0 20px ${p.color}11` }}
                onMouseOver={(e) => { e.currentTarget.style.borderColor = p.color; e.currentTarget.style.boxShadow = `0 0 30px ${p.color}33`; }}
                onMouseOut={(e) => { e.currentTarget.style.borderColor = `${p.color}33`; e.currentTarget.style.boxShadow = `0 0 20px ${p.color}11`; }}>
                <div style={{ fontSize: "16px", marginBottom: "8px", fontWeight: "bold" }}>{p.label}</div>
                <div style={{ color: "#888", fontSize: "11px", lineHeight: "1.5" }}>{p.desc}</div>
                <div style={{ color: "#555", fontSize: "10px", marginTop: "8px" }}>{p.levels().length} niveaux • Randomisé</div>
              </button>
            ))}
          </div>
          {leaderboard.length > 0 && (
            <div style={{ marginTop: "40px", maxWidth: "400px", margin: "40px auto 0" }}>
              <div style={{ color: "#ff0", fontSize: "12px", letterSpacing: "2px", marginBottom: "12px", textAlign: "left" }}>🏆 LEADERBOARD</div>
              {leaderboard.slice(0, 8).map((e, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", color: i === 0 ? "#ff0" : "#666", fontSize: "12px", padding: "4px 0", borderBottom: "1px solid #111" }}>
                  <span>{i + 1}. {e.name}</span><span>{e.score} pts — {e.profile}</span>
                </div>
              ))}
            </div>
          )}
        </div>
      </div>
    );
  }

  // ─── VICTORY ───
  if (screen === "victory") {
    const elapsed = Math.floor((Date.now() - startTime) / 1000);
    const score = getScore();
    const p = PROFILES[profile];
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Fira Code',monospace", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
        <MatrixRain />
        <div style={{ zIndex: 10, textAlign: "center", padding: "50px", border: `1px solid ${p.color}44`, borderRadius: "8px", background: "rgba(0,0,0,0.9)", boxShadow: `0 0 60px ${p.color}15` }}>
          <div style={{ fontSize: "48px", marginBottom: "16px" }}>🏆</div>
          <h1 style={{ color: p.color, fontSize: "26px", margin: "0 0 6px", letterSpacing: "4px" }}>MISSION ACCOMPLIE</h1>
          <div style={{ color: "#555", fontSize: "12px", letterSpacing: "3px", marginBottom: "30px" }}>{p.label}</div>
          <div style={{ color: "#888", marginBottom: "30px", lineHeight: "2.2", fontSize: "14px" }}>
            <div>Niveaux : <span style={{ color: p.color }}>{levels.length}/{levels.length}</span></div>
            <div>Temps : <span style={{ color: p.color }}>{Math.floor(elapsed / 60)}m {String(elapsed % 60).padStart(2, "0")}s</span></div>
            <div>Indices : <span style={{ color: hintsUsed === 0 ? "#0f0" : "#ff0" }}>{hintsUsed}</span></div>
            <div style={{ fontSize: "22px", marginTop: "8px" }}>Score : <span style={{ color: p.color }}>{score}</span></div>
          </div>
          <div style={{ color: "#444", fontSize: "11px", marginBottom: "24px", maxWidth: "380px", lineHeight: "1.6" }}>
            {levels.map((l) => l.topic).join(" · ")}
          </div>
          <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => {
              setLeaderboard((prev) => [...prev, { name: playerName || "Anonyme", score, profile: p.label, time: elapsed }].sort((a, b) => b.score - a.score));
              setScreen("menu");
            }} style={{ background: p.color, color: "#000", border: "none", padding: "12px 28px", fontFamily: "inherit", fontSize: "13px", fontWeight: "bold", cursor: "pointer", letterSpacing: "2px", borderRadius: "2px" }}>
              SAUVER & MENU
            </button>
            <button onClick={() => startGame(profile)}
              style={{ background: "transparent", color: p.color, border: `1px solid ${p.color}`, padding: "12px 28px", fontFamily: "inherit", fontSize: "13px", cursor: "pointer", letterSpacing: "2px", borderRadius: "2px" }}>
              REJOUER
            </button>
          </div>
        </div>
      </div>
    );
  }

  // ─── GAME ───
  const p = PROFILES[profile];
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Fira Code',monospace", display: "flex", flexDirection: "column", position: "relative" }} onClick={focusInput}>
      <div style={{ background: "#111", borderBottom: "1px solid #1a1a1a", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px", zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          <span style={{ color: p.color, fontSize: "13px", fontWeight: "bold" }}>⬡ CYBER TERMINAL</span>
          <span style={{ color: "#333" }}>|</span>
          <span style={{ color: p.color, fontSize: "11px" }}>{p.label}</span>
          <span style={{ color: "#333" }}>|</span>
          <span style={{ color: "#0a0", fontSize: "11px" }}>{levels[levelIdx]?.title}</span>
        </div>
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          {playerName && <span style={{ color: "#555", fontSize: "11px" }}>{playerName}</span>}
          <span style={{ color: "#555", fontSize: "11px" }}>LVL {levelIdx + 1}/{levels.length}</span>
          <div style={{ display: "flex", gap: "3px" }}>
            {levels.map((_, i) => (
              <div key={i} style={{ width: "7px", height: "7px", borderRadius: "50%", background: i < levelIdx ? "#0f0" : i === levelIdx ? p.color : "#222", boxShadow: i < levelIdx ? "0 0 4px #0f0" : i === levelIdx ? `0 0 6px ${p.color}` : "none", transition: "all 0.3s" }} />
            ))}
          </div>
          <button onClick={() => setScreen("menu")} style={{ background: "none", border: "1px solid #333", color: "#555", padding: "2px 8px", fontFamily: "inherit", fontSize: "10px", cursor: "pointer", borderRadius: "2px" }}>MENU</button>
        </div>
      </div>
      <div ref={termRef} style={{ flex: 1, overflowY: "auto", padding: "14px 16px", paddingBottom: "65px" }}>
        {lines.map((line, i) => {
          if (line.type === "empty") return <div key={i} style={{ height: "10px" }} />;
          const colors = { system: "#0a0", input: "#fff", success: "#0f0", error: "#f44", hint: "#ff0", mission: "#6af", response: "#aaa" };
          return <div key={i} style={{ color: colors[line.type] || "#aaa", fontSize: "13px", lineHeight: "1.6", whiteSpace: "pre-wrap", fontFamily: "inherit" }}>{line.text}</div>;
        })}
      </div>
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#111", borderTop: "1px solid #1a1a1a", padding: "10px 16px", display: "flex", alignItems: "center", gap: "8px", zIndex: 10 }}>
        <span style={{ color: p.color, fontSize: "14px" }}>❯❯</span>
        <input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} autoFocus placeholder="Tapez une commande..."
          style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#fff", fontSize: "14px", fontFamily: "inherit", caretColor: p.color }} />
        <span style={{ color: "#333", fontSize: "10px" }}>ENTER ↵</span>
      </div>
    </div>
  );
}
